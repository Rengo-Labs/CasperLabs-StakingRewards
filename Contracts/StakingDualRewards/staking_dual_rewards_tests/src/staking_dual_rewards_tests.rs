...
fn deploy() -> (TestEnv, AccountHash, TestContract, TestContract) {
    ...
    let proxy =
        STAKINGDUALREWARDSInstance::proxy(&env, "Proxy", owner, staking_dual_rewards_package_hash);
    // For Minting Purpose
...
fn reward_per_token_a() {
    ...
    staking_dual_rewards_instance.stake(owner, amount);
    staking_dual_rewards_instance.notify_reward_amount(
        owner,
        U256::from(TEN_E_NINE * 20),
        U256::from(TEN_E_NINE * 20),
        100.into(),
    );
...
fn recover_erc20() {
    ...
    let staking_dual_rewards_package_hash: ContractPackageHash =
        instance.query_named_key("self_package_hash".to_string());
    let to: Key = Key::from(staking_dual_rewards_package_hash);
    let staking_dual_rewards_instance = STAKINGDUALREWARDSInstance::contract_instance(instance);
    ...
    staking_dual_rewards_instance.stake(owner, amount);
    staking_dual_rewards_instance.recover_erc20(
        owner,
        Key::Hash(token_address.package_hash()),
        amount,
    );
}
